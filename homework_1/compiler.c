...
// =========================================================
typedef enum {
    TK_FUNC, TK_RETURN, TK_IF, TK_ID, TK_NUM, TK_WHILE, 
...
// =========================================================
int t_idx = 0;
void new_t(char *s) { snprintf(s, 32, "t%d", ++t_idx); } // 生成唯一暫存變數名
...
void statement() {
    ...
    if (cur_token.type == TK_IF) {
        ...
        next_token(); 
        snprintf(quads[jmp_idx].result, sizeof(quads[jmp_idx].result), "%d", quad_count); // 回填真實跳轉地址
    // EBNF: assignment_statement = identifier "=" expression [ ";" ] ;
        ...
        int start_idx = quad_count; // 1. 記下迴圈條件判斷的起點
        char cond[32];
        expression(cond);           // 2. 解析條件式
        next_token();               // 跳過 ')'
        if (cur_token.type == TK_LBRACE) {
            next_token();           // 跳過 '{' (防呆，確保真的是大括號)
        }
        
        int jmpf_idx = quad_count;  // 3. 記下 JMP_F 的位置準備回填
        emit("JMP_F", cond, "-", "?");
        ...
        // 5. 無條件跳回起點 (因為 start_idx 已經知道了，直接轉字串 Emit，不需回填)
        char buf[32];
        snprintf(buf, sizeof(buf), "%d", start_idx);
        emit("JMP", "-", "-", buf); 
        
        // 6. 回填 JMP_F 的目標地址 (迴圈結束後的下一個指令位置 quad_count)
        snprintf(quads[jmpf_idx].result, sizeof(quads[jmpf_idx].result), "%d", quad_count);
